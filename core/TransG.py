...
class TransG(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            self.ent_embeddings_var = tf.compat.v1.get_variable(name="ent_embedding_var", shape=[num_total_ent],
                                                  initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))

            self.rel_clusters = tf.compat.v1.get_variable(name="rel_clusters", shape=[num_total_rel, self.config.ncluster, k],
                                                  initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))

            self.weights_clusters = tf.compat.v1.get_variable(name="weights_clusters", shape=[num_total_rel, self.config.ncluster],
                                              initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def def_loss(self):
        ...
        pos_h_e = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.pos_h)
        pos_t_e = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.pos_t)
        pos_r_c_e = tf.nn.embedding_lookup(params=norm_rel_clusters, ids=self.pos_r)
        pos_r_w_e = tf.nn.embedding_lookup(params=self.weights_clusters, ids=self.pos_r)
        ...
        pos_exp_e = tf.math.exp(tf.negative(tf.reduce_sum(input_tensor=pos_exp_e ** 2, axis=2)))
        pos_score = tf.reduce_sum(input_tensor=pos_exp_e * pos_r_w_e, axis=1)

        neg_h_e = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.neg_h)
        neg_t_e = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.neg_t)
        neg_r_c_e = tf.nn.embedding_lookup(params=norm_rel_clusters, ids=self.neg_r)
        neg_r_w_e = tf.nn.embedding_lookup(params=self.weights_clusters, ids=self.neg_r)

        neg_h_e = tf.expand_dims(neg_h_e, axis=1)
        neg_t_e = tf.expand_dims(neg_t_e, axis=1)

        neg_exp_e = (neg_h_e - neg_t_e) + neg_r_c_e
        neg_exp_e = tf.math.exp(tf.negative(tf.reduce_sum(input_tensor=neg_exp_e ** 2, axis=2)))
        neg_score = tf.reduce_sum(input_tensor=neg_exp_e * neg_r_w_e, axis=1)
        
        # regularizers 
        self.loss = 0.001 * tf.reduce_sum(input_tensor=norm_ent_embeddings**2) + tf.reduce_sum(input_tensor=norm_rel_clusters**2)
        self.loss = tf.reduce_sum(input_tensor=tf.maximum(- pos_score + self.config.margin + neg_score, 0))
        # import pdb; pdb.set_trace()
        # tf.math.exp
    ...
    def test_batch(self):
        ...
        head_vec = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.test_h_batch)
        tail_vec = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.test_t_batch) 

        rel_c_vec = tf.nn.embedding_lookup(params=norm_rel_clusters, ids=self.test_r_batch) # ?, 4, 50
        rel_r_w_vec = tf.nn.embedding_lookup(params=self.weights_clusters, ids=self.test_r_batch) # ?, 4
        ...
        head_exp_e = tf.math.exp(tf.negative(tf.reduce_sum(input_tensor=head_exp_e ** 2, axis=-1))) # ?, 14951, 4
        head_score = tf.reduce_sum(input_tensor=head_exp_e*rel_r_w_vec, axis=-1)

        tail_exp_e = (head_vec - all_ent_embeddings) + rel_c_vec # ?, 14951, 4, 50
        tail_exp_e = tf.math.exp(tf.negative(tf.reduce_sum(input_tensor=tail_exp_e ** 2, axis=-1))) # ?, 14951, 4
        tail_score = tf.reduce_sum(input_tensor=tail_exp_e*rel_r_w_vec, axis=-1)
