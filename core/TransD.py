...
class TransD(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            self.ent_embeddings = tf.compat.v1.get_variable(name="ent_embedding",
            ...
            self.rel_embeddings = tf.compat.v1.get_variable(name="rel_embedding",
                                                  shape=[num_total_rel, k],
                                                  initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.ent_mappings = tf.compat.v1.get_variable(name="ent_mappings",
                                                shape=[num_total_ent, d],
                                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.rel_mappings = tf.compat.v1.get_variable(name="rel_mappings",
                                                shape=[num_total_rel, k],
                                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def def_loss(self):
        ...
        pos_h_e = tf.nn.l2_normalize(pos_h_e + tf.reduce_sum(input_tensor=pos_h_e * pos_h_m, axis=-1, keepdims=True) * pos_r_m, -1)
        pos_r_e = tf.nn.l2_normalize(pos_r_e, -1)
        pos_t_e = tf.nn.l2_normalize(pos_t_e + tf.reduce_sum(input_tensor=pos_t_e * pos_t_m, axis=-1, keepdims=True) * pos_r_m, -1)

        neg_h_e = tf.nn.l2_normalize(neg_h_e + tf.reduce_sum(input_tensor=neg_h_e * neg_h_m, axis=-1, keepdims=True) * neg_r_m, -1)
        neg_r_e = tf.nn.l2_normalize(neg_r_e, -1)
        neg_t_e = tf.nn.l2_normalize(neg_t_e + tf.reduce_sum(input_tensor=neg_t_e * neg_t_m, axis=-1, keepdims=True) * neg_r_m, -1)

        score_pos = self.distance(pos_h_e, pos_r_e, pos_t_e)
        score_neg = self.distance(neg_h_e, neg_r_e, neg_t_e)

        self.loss = tf.reduce_sum(input_tensor=tf.maximum(score_pos - score_neg + self.config.margin, 0))
    ...
    def test_batch(self):
       ...
       """
        ...
        head_vec = tf.nn.l2_normalize(head_vec + tf.reduce_sum(input_tensor=head_vec * h_m, axis=-1, keepdims=True) * r_m, -1)
        rel_vec = tf.nn.l2_normalize(rel_vec, -1)
        tail_vec = tf.nn.l2_normalize(tail_vec + tf.reduce_sum(input_tensor=tail_vec * t_m, axis=-1, keepdims=True) * r_m, -1)
        ...
        project_ent_embedding = tf.nn.l2_normalize(
            norm_ent_embeddings + tf.reduce_sum(input_tensor=norm_ent_embeddings * norm_ent_mappings, axis=-1, keepdims=True) * tf.expand_dims(r_m, axis=1), -1)
    ...
    def distance(self, h, r, t, axis=1):
       ...
       """
        if self.config.L1_flag:
            return tf.reduce_sum(input_tensor=tf.abs(h + r - t), axis=axis)  # L1 norm
        else:
            return tf.reduce_sum(input_tensor=(h + r - t) ** 2, axis=axis)  # L2 norm
    ...
    def get_mapping(self, h, r, t):
        ...
        """
        h_m = tf.nn.embedding_lookup(params=self.ent_mappings, ids=h)
        r_m = tf.nn.embedding_lookup(params=self.rel_mappings, ids=r)
        t_m = tf.nn.embedding_lookup(params=self.ent_mappings, ids=t)
    ...
    def embed(self, h, r, t):
        ...
        """
        h_e = tf.nn.embedding_lookup(params=self.ent_embeddings, ids=h)
        r_e = tf.nn.embedding_lookup(params=self.rel_embeddings, ids=r)
        t_e = tf.nn.embedding_lookup(params=self.ent_embeddings, ids=t)
