...
class TransH(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            self.w = tf.compat.v1.get_variable(name="w", shape=[num_total_rel, k],
                                     initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def def_loss(self):
        ...
        self.loss = tf.reduce_sum(input_tensor=tf.maximum(0., score_pos + self.config.margin - score_neg)) + self.get_reg()
    ...
    def get_reg(self):
        ...
        term1 = tf.reduce_sum(input_tensor=tf.maximum(tf.reduce_sum(input_tensor=norm_ent_embedding ** 2, axis=-1) - 1, 0))
        term2 = tf.reduce_sum(input_tensor=tf.maximum(tf.compat.v1.div(tf.reduce_sum(input_tensor=norm_rel_embedding * norm_w, axis=-1) ** 2,
                                                tf.reduce_sum(input_tensor=norm_rel_embedding ** 2, axis=-1)) - 1e-07, 0))
    ...
    def get_proj(self, r):
        ...
        return tf.nn.l2_normalize(tf.nn.embedding_lookup(params=self.w, ids=r), axis=-1)

    def projection(self, entity, wr):
        """Calculates the projection of entities"""
        return entity - tf.reduce_sum(input_tensor=entity * wr, axis=-1, keepdims=True) * wr
