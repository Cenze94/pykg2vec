...
class TransM(ModelMeta):
    ...
    def def_loss(self):
        ...
        pos_r_theta = tf.nn.embedding_lookup(params=self.theta, ids=self.pos_r)
        neg_r_theta = tf.nn.embedding_lookup(params=self.theta, ids=self.neg_r)
    ...
    def distance(self, h, r, t):
        ...
        if self.config.L1_flag:
            return tf.reduce_sum(input_tensor=tf.abs(h + r - t), axis=-1)  # L1 norm
        else:
            return tf.reduce_sum(input_tensor=(h + r - t) ** 2, axis=-1)  # L2 norm
