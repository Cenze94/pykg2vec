...
class RotatE(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            self.ent_embeddings_real = tf.compat.v1.get_variable(name="ent_embeddings_real", shape=[num_total_ent, k],
            ...
            self.ent_embeddings_imag = tf.compat.v1.get_variable(name="ent_embeddings_imag", shape=[num_total_ent, k],
                                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.rel_embeddings_real = tf.compat.v1.get_variable(name="rel_embeddings_real", shape=[num_total_rel, k],
                                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def comp_mul_and_min(self, hr, hi, rr, ri, tr, ti):
        ...
        return tf.reduce_sum(input_tensor=tf.sqrt(score_r ** 2 + score_i ** 2), axis=-1)
    ...
    def comp_mul_and_min_4_test(self, hr, hi, rr, ri, tr, ti):
        ...
        score_r = tf.cond(pred=tf.shape(input=hr)[0] < tf.shape(input=tr)[0],
                          true_fn=lambda: tf.expand_dims(hr, axis=1)*rr-tf.expand_dims(hi, axis=1)*ri-tr,
                          false_fn=lambda: hr*rr-hi*ri-tf.expand_dims(tr, axis=1))

        score_i = tf.cond(pred=tf.shape(input=hr)[0] < tf.shape(input=tr)[0],
                          true_fn=lambda: tf.expand_dims(hr, axis=1) * ri + tf.expand_dims(hi, axis=1) * rr - ti,
                          false_fn=lambda:  hr * ri + hi * rr - tf.expand_dims(ti, axis=1))
    ...
    def embed(self, h, r, t):
        ...
        h_e_r = tf.nn.embedding_lookup(params=self.ent_embeddings_real, ids=h)
        h_e_i = tf.nn.embedding_lookup(params=self.ent_embeddings_imag, ids=h)
        r_e_r = tf.nn.embedding_lookup(params=self.rel_embeddings_real, ids=r)
        t_e_r = tf.nn.embedding_lookup(params=self.ent_embeddings_real, ids=t)
        t_e_i = tf.nn.embedding_lookup(params=self.ent_embeddings_imag, ids=t)
        r_e_r = r_e_r / pi
        r_e_i = tf.sin(r_e_r)
        r_e_r = tf.cos(r_e_r)
