...
class Complex(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            self.emb_e_real = tf.compat.v1.get_variable(name="emb_e_real", shape=[self.tot_ent, k],
            ...
            self.emb_e_img = tf.compat.v1.get_variable(name="emb_e_img", shape=[self.tot_ent, k],
                                             initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.emb_rel_real = tf.compat.v1.get_variable(name="emb_rel_real", shape=[self.tot_rel, k],
                                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.emb_rel_img = tf.compat.v1.get_variable(name="emb_rel_img", shape=[self.tot_rel, k],
                                               initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def def_loss(self):
        ...
        realrealreal = tf.matmul(h_emb_real * r_emb_real,
                                 tf.transpose(a=tf.nn.l2_normalize(self.emb_e_real, axis=1)))
        realimgimg = tf.matmul(h_emb_real * r_emb_img,
                               tf.transpose(a=tf.nn.l2_normalize(self.emb_e_img, axis=1)))
    ...
    def embed(self, h, r, t):
        ...
        h_emb_real = tf.nn.embedding_lookup(params=norm_emb_e_real, ids=h)
        t_emb_real = tf.nn.embedding_lookup(params=norm_emb_e_real, ids=t)

        h_emb_img = tf.nn.embedding_lookup(params=norm_emb_e_img, ids=h)
        t_emb_img = tf.nn.embedding_lookup(params=norm_emb_e_img, ids=t)

        r_emb_real = tf.nn.embedding_lookup(params=norm_emb_rel_real, ids=r)
        r_emb_img = tf.nn.embedding_lookup(params=norm_emb_rel_img, ids=r)
