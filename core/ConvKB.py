...
class ConvKB(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            self.rel_embeddings = tf.compat.v1.get_variable(name="rel_embedding", shape=[num_total_rel, k],
                                                  initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
        with tf.compat.v1.name_scope("activation_bias"):
            self.b = tf.compat.v1.get_variable(name="bias", shape=[1, num_total_ent],
                                     initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def forward(self, x, batch):
        ...
        x = tf.matmul(x, tf.transpose(a=tf.nn.l2_normalize(self.ent_embeddings, axis=1)))
        #add a bias value
        x = tf.add(x, self.b)
        # sigmoid activation
    ...
    def def_loss(self):
        ...
        h_emb = tf.nn.embedding_lookup(params=ent_emb_norm, ids=self.h)
        r_emb = tf.nn.embedding_lookup(params=rel_emb_norm, ids=self.r)
        t_emb = tf.nn.embedding_lookup(params=ent_emb_norm, ids=self.t)
        ...
        loss_tail_pred = tf.reduce_mean(input_tensor=tf.keras.backend.binary_crossentropy(hr_t, pred_tails))
        loss_head_pred = tf.reduce_mean(input_tensor=tf.keras.backend.binary_crossentropy(rt_h, pred_heads))
    ...
    def test_batch(self):
        ...
        h_emb = tf.nn.embedding_lookup(params=ent_emb_norm, ids=self.test_h_batch)
        r_emb = tf.nn.embedding_lookup(params=rel_emb_norm, ids=self.test_r_batch)
        t_emb = tf.nn.embedding_lookup(params=ent_emb_norm, ids=self.test_t_batch)
