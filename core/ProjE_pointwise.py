...
class ProjE_pointwise(ModelMeta):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            self.bc1 = tf.compat.v1.get_variable(name="bc1", shape=[k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.De1 = tf.compat.v1.get_variable(name="De1", shape=[k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.Dr1 = tf.compat.v1.get_variable(name="Dr1", shape=[k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))

            self.bc2 = tf.compat.v1.get_variable(name="bc2", shape=[k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.De2 = tf.compat.v1.get_variable(name="De2", shape=[k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.Dr2 = tf.compat.v1.get_variable(name="Dr2", shape=[k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
    ...
    def def_loss(self):
        ...
        emb_hr_h = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.h)  # [m, k]
        emb_hr_r = tf.nn.embedding_lookup(params=norm_rel_embeddings, ids=self.r)  # [m, k]

        emb_tr_t = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.t)  # [m, k]
        emb_tr_r = tf.nn.embedding_lookup(params=norm_rel_embeddings, ids=self.r)  # [m, k]

        hrt_sigmoid = self.g(tf.nn.dropout(self.f1(emb_hr_h, emb_hr_r), 1 - (0.5)), norm_ent_embeddings)

        hrt_loss_left = - tf.reduce_sum(input_tensor=(tf.math.log(tf.clip_by_value(hrt_sigmoid, 1e-10, 1.0)) * tf.maximum(0., self.hr_t)))
        hrt_loss_right = - tf.reduce_sum(
            input_tensor=(tf.math.log(tf.clip_by_value(1 - hrt_sigmoid, 1e-10, 1.0)) * tf.maximum(0., tf.negative(self.hr_t))))

        hrt_loss = hrt_loss_left + hrt_loss_right

        trh_sigmoid = self.g(tf.nn.dropout(self.f2(emb_tr_t, emb_tr_r), 1 - (0.5)), norm_ent_embeddings)

        trh_loss_left = - tf.reduce_sum(input_tensor=(tf.math.log(tf.clip_by_value(trh_sigmoid, 1e-10, 1.0)) * tf.maximum(0., self.rt_h)))
        trh_loss_right = - tf.reduce_sum(
            input_tensor=(tf.math.log(tf.clip_by_value(1 - trh_sigmoid, 1e-10, 1.0)) * tf.maximum(0., tf.negative(self.rt_h))))

        trh_loss = trh_loss_left + trh_loss_right

        regularizer_loss = tf.reduce_sum(input_tensor=tf.abs(self.De1) + tf.abs(self.Dr1)) + tf.reduce_sum(input_tensor=tf.abs(self.De2) + tf.abs(self.Dr2)) \
                            + tf.reduce_sum(input_tensor=tf.abs(self.ent_embeddings)) + tf.reduce_sum(input_tensor=tf.abs(self.rel_embeddings))
    ...
    def g(self, f, W):
        ...
        """
        return tf.sigmoid(tf.matmul(f, tf.transpose(a=W)))
    ...
    def test_batch(self):
        ...
        h_vec = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.test_h_batch)  # [1, k]
        r_vec = tf.nn.embedding_lookup(params=norm_rel_embeddings, ids=self.test_r_batch)  # [1, k]
        t_vec = tf.nn.embedding_lookup(params=norm_ent_embeddings, ids=self.test_t_batch)  # [1, k]
    ...
    def embed(self, h, r, t):
        ...
        """
        emb_h = tf.nn.embedding_lookup(params=self.ent_embeddings, ids=h)
        emb_r = tf.nn.embedding_lookup(params=self.rel_embeddings, ids=r)
        emb_t = tf.nn.embedding_lookup(params=self.ent_embeddings, ids=t)
