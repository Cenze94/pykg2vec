...
class SME(ModelMeta):
    ...
    def gu_bilinear(self, h, r, test_flag):
       ...
       """
        if test_flag:
            tmp1 = tf.cond(pred=tf.shape(input=h)[0] > tf.shape(input=r)[0],
                           true_fn=lambda: tf.expand_dims(tf.transpose(a=tf.matmul(self.mu2, tf.transpose(a=r))), axis=1),
                           false_fn=lambda: tf.transpose(a=tf.matmul(self.mu2, tf.transpose(a=r))))
            return tf.multiply(tf.transpose(a=tf.matmul(self.mu1, tf.transpose(a=h))), tmp1) + tf.squeeze(self.bu, axis=-1)
        else:
            return tf.transpose(
                a=tf.multiply(tf.matmul(self.mu1, tf.transpose(a=h)), tf.matmul(self.mu2, tf.transpose(a=r))) + self.bu)
    ...
    def gv_bilinear(self, r, t, test_flag):
        ...
        """
        if test_flag:
            tmp1 = tf.cond(pred=tf.shape(input=t)[0] > tf.shape(input=r)[0],
                           true_fn=lambda: tf.expand_dims(tf.transpose(a=tf.matmul(self.mv1, tf.transpose(a=r))), axis=1),
                           false_fn=lambda: tf.transpose(a=tf.matmul(self.mv1, tf.transpose(a=r))))
            return tf.multiply(tf.transpose(a=tf.matmul(self.mv2, tf.transpose(a=t))), tmp1) + tf.squeeze(self.bu, axis=-1)
        else:
            return tf.transpose(
                a=tf.multiply(tf.matmul(self.mv1, tf.transpose(a=r)), tf.matmul(self.mv2, tf.transpose(a=t))) + self.bv)
    ...
    def gu_linear(self, h, r, test_flag):
        ...
        if test_flag:
            tmp1 = tf.cond(pred=tf.shape(input=h)[0] > tf.shape(input=r)[0],
                           true_fn=lambda: tf.expand_dims(tf.transpose(a=tf.matmul(self.mu2, tf.transpose(a=r)) + self.bu), axis=1),
                           false_fn=lambda: tf.transpose(a=tf.matmul(self.mu2, tf.transpose(a=r)) + self.bu))
            return tf.transpose(a=tf.matmul(self.mu1, tf.transpose(a=h))) + tmp1
        else:
            return tf.transpose(a=tf.matmul(self.mu1, tf.transpose(a=h)) + tf.matmul(self.mu2, tf.transpose(a=r)) + self.bu)
    ...
    def gv_linear(self, r, t, test_flag):
        ...
        if test_flag:
            tmp1 = tf.cond(pred=tf.shape(input=t)[0] > tf.shape(input=r)[0],
                           true_fn=lambda: tf.expand_dims(tf.transpose(a=tf.matmul(self.mv1, tf.transpose(a=r)) + self.bv), axis=1),
                           false_fn=lambda: tf.transpose(a=tf.matmul(self.mv1, tf.transpose(a=r)) + self.bv))
            return tf.transpose(a=tf.matmul(self.mv2, tf.transpose(a=t))) + tmp1
        else:
            return tf.transpose(a=tf.matmul(self.mv1, tf.transpose(a=r)) + tf.matmul(self.mv2, tf.transpose(a=t)) + self.bv)
       ...
       """
        if self.config.bilinear:
            if test_flag:
                ...
                result = tf.cond(pred=tf.shape(input=tmp1)[1] < tf.shape(input=tmp2)[1],
                                 true_fn=lambda: tf.reduce_sum(input_tensor=tf.multiply(tf.expand_dims(tmp1, axis=1), tmp2), axis=-1),
                                 false_fn=lambda: tf.reduce_sum(input_tensor=tf.multiply(tf.expand_dims(tmp2, axis=1), tmp1), axis=-1))
                return result
            else:
                return tf.reduce_sum(input_tensor=tf.multiply(self.gu_bilinear(h, r, test_flag), self.gv_bilinear(r, t, test_flag)),
                                     axis=1)
        else:
            if test_flag:
                ...
                result = tf.cond(pred=tf.shape(input=tmp1)[1] < tf.shape(input=tmp2)[1],
                                 true_fn=lambda: tf.reduce_sum(input_tensor=tf.expand_dims(tmp1, axis=1) * tmp2, axis=-1),
                                 false_fn=lambda: tf.reduce_sum(input_tensor=tf.expand_dims(tmp2, axis=1) * tmp1, axis=-1))
                return result
            else:
                return tf.reduce_sum(input_tensor=self.gu_linear(h, r, test_flag) * self.gv_linear(r, t, test_flag), axis=1)
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("weights_and_parameters"):
            self.mu1 = tf.compat.v1.get_variable(name="mu1", shape=[k, k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.mu2 = tf.compat.v1.get_variable(name="mu2", shape=[k, k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.bu = tf.compat.v1.get_variable(name="bu", shape=[k, 1],
                                      initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.mv1 = tf.compat.v1.get_variable(name="mv1", shape=[k, k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.mv2 = tf.compat.v1.get_variable(name="mv2", shape=[k, k],
                                       initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
            self.bv = tf.compat.v1.get_variable(name="bv", shape=[k, 1],
                                      initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal")))
