...
class SLM(ModelMeta):
    ...
    def def_loss(self):
        ...
        energy_pos = tf.reduce_sum(input_tensor=pos_r_e * self.layer(pos_h_e, pos_t_e), axis=-1)
        energy_neg = tf.reduce_sum(input_tensor=neg_r_e * self.layer(neg_h_e, neg_t_e), axis=-1)
    ...
    def test_layer(self, h, t):
        ...
        result = tf.cond(pred=tf.shape(input=mr1h)[0] < tf.shape(input=mr2t)[0], true_fn=lambda: tf.tanh(tf.expand_dims(mr1h, axis=1) + mr2t),
                         false_fn=lambda: tf.tanh(tf.expand_dims(mr2t, axis=1) + mr1h))
    ...
    def test_batch(self):
        ...
        energy_h = tf.reduce_sum(input_tensor=tf.expand_dims(r_vec,axis=1) * self.test_layer(tf.nn.l2_normalize(self.ent_embeddings, axis=1), t_vec), axis=-1)
        energy_t = tf.reduce_sum(input_tensor=tf.expand_dims(r_vec,axis=1) * self.test_layer(h_vec, tf.nn.l2_normalize(self.ent_embeddings, axis=1)), axis=-1)
