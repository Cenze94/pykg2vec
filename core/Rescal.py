...
class Rescal(ModelMeta):
    ...
    def def_inputs(self):
        ...
        """
        with tf.compat.v1.name_scope("read_inputs"):
    ...
    def def_parameters(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            self.rel_matrices = tf.compat.v1.get_variable(name="rel_matrices",
    ...
    def cal_truth_val(self, h, r, t):
        ...
        #        r: [m, k, k]
        #        t: [m, k, 1]
        return tf.reduce_sum(input_tensor=h * tf.matmul(r, t), axis=[1, 2])

    def def_loss(self):
        """Defines the loss function for the algorithm."""
        k = self.config.hidden_size

        with tf.compat.v1.name_scope('normalization'):
        ...
        with tf.compat.v1.name_scope('lookup_embeddings'):
            pos_h_e = tf.nn.embedding_lookup(params=self.ent_embeddings, ids=self.pos_h)
            pos_r_e = tf.nn.embedding_lookup(params=self.rel_matrices, ids=self.pos_r)
            ...
            neg_r_e = tf.nn.embedding_lookup(params=self.rel_matrices, ids=self.neg_r)
            neg_t_e = tf.nn.embedding_lookup(params=self.ent_embeddings, ids=self.neg_t)

        with tf.compat.v1.name_scope('reshaping'):
        ...
        self.loss = tf.reduce_sum(input_tensor=tf.maximum(neg_score + self.config.margin - pos_score, 0))
    ...
    def embed(self, h, r, t):
        ...
        emb_r = tf.nn.embedding_lookup(params=tf.nn.l2_normalize(self.rel_matrices, axis=1), ids=r)
