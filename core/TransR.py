...
class TransR(ModelMeta):
    ...
    def test_batch(self):
        ...
        project_ent_embedding = self.transform(self.ent_embeddings, tf.transpose(a=pos_matrix))
    ...
    def get_transform_matrix(self, r):
        ...
        d = self.config.ent_hidden_size
        k = self.config.rel_hidden_size
        return tf.reshape(tf.nn.embedding_lookup(params=self.rel_matrix, ids=r), [-1, k, d])
    ...
    def embed(self, h, r, t):
        ...
        h_e = tf.reshape(tf.nn.embedding_lookup(params=self.ent_embeddings, ids=h), [-1, d, 1])
        r_e = tf.reshape(tf.nn.embedding_lookup(params=self.rel_embeddings, ids=r), [-1, k])
        t_e = tf.reshape(tf.nn.embedding_lookup(params=self.ent_embeddings, ids=t), [-1, d, 1])
        matrix = tf.reshape(tf.nn.embedding_lookup(params=self.rel_matrix, ids=r), [-1, k, d])
